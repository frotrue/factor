...
export default class WaveManager {
    ...
    spawnEnemy() {
        ...
        switch (edge) {
            case 0:
                x = -mapSize / 2 + Math.random() * mapSize;
                y = -mapSize / 2;
                break;
            case 1:
                x = mapSize / 2;
                y = -mapSize / 2 + Math.random() * mapSize;
                break;
            case 2:
                x = -mapSize / 2 + Math.random() * mapSize;
                y = mapSize / 2;
        ...
        if (isBossWave && this.enemiesSpawned === this.enemiesToSpawn) {
            ...
            if (uiManager)
                uiManager.logMessage('System: WARNING - Overfitted Model detected!', true);
        }
        else {
            if (this.currentWave > 5 && Math.random() < 0.3)
                type = 'MALWARE';
            if (this.currentWave > 15 && Math.random() < 0.2)
                type = 'ADVERSARIAL';
        }
