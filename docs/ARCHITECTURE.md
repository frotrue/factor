...
- DOM UI: `src/managers/UIManager.ts`와 하위 UI 매니저, `index.html`, `src/styles/main.css`, `src/styles/legacy-ui.css`
- Preact HUD overlay: `src/ui/mountHud.tsx`, `src/ui/HudApp.tsx`, `src/ui/signals/*`, `src/ui/components/MainMenu.tsx`, `src/ui/components/TopBar.tsx`, `src/ui/components/RightRail.tsx`, `src/ui/components/BuildConsole.tsx`, `src/ui/components/SettingsModal.tsx`, `src/ui/components/TrainingLabModal.tsx`, `src/ui/components/GameOverScreen.tsx`, `src/ui/components/WaveResultCard.tsx`, `src/ui/components/ActivityLog.tsx`, `src/ui/components/Tooltip.tsx`, `src/ui/components/TutorialPanel.tsx`, `src/ui/components/MobileActionBar.tsx`, `src/ui/shared/*`, `src/ui/legacyMainMenuFallback.ts`, `src/ui/topHudDisplay.ts`, `src/ui/buildConsoleSnapshot.ts`, `src/ui/tacticalPanelDisplay.ts`, `src/ui/settingsDisplay.ts`, `src/ui/trainingLabDisplay.ts`, `src/ui/mobileActionDisplay.ts`, `src/ui/waveResultDisplay.ts`, `src/ui/gameOverDisplay.ts`, `src/ui/notificationDisplay.ts`, `src/ui/legacyHudDom.ts`, `src/ui/legacyTopHud.ts`, `src/ui/legacyTacticalPanels.ts`, `src/ui/legacyBuildConsole.ts`, `src/ui/legacySettings.ts`, `src/ui/legacyTrainingLab.ts`, `src/ui/legacyTutorialPanel.ts`, `src/ui/legacyMobileControls.ts`, `src/ui/legacyGameOver.ts`, `src/ui/legacyNotifications.ts`, `src/ui/domEnvironment.ts`, `src/styles/tokens.css`

## 화면/UI 흐름

1. `MainMenuScene`이 메뉴 배경 그리드/파티클을 Phaser로 렌더링하고, 보이는 타이틀/난이도/설명/시작/이어하기 UI는 Preact MainMenu overlay가 표시합니다. 기존 Phaser 텍스트 객체와 입력 hit target은 삭제하지 않고 `src/ui/legacyMainMenuFallback.ts`가 투명 fallback으로 생성하며, `src/ui/mainMenuDisplay.ts`가 legacy fallback 입력과 overlay snapshot 공통 display payload 및 localized aria label을 만듭니다. 저장 슬롯이 있으면 Preact 이어하기 버튼이 `loadSave` start request를 보내고, legacy gameplay HUD display 복원은 `src/ui/legacyHudDom.ts`에 위임한 뒤 `MainScene` 초기화 완료 후 기존 `SaveManager.loadGame()` 경로가 복원합니다.
2. 앱 시작 시 `src/main.ts`가 Phaser Game을 만든 뒤 `#preact-hud`에 Preact HUD overlay를 마운트합니다. `src/ui/signals/bridge.ts`는 EventBus의 menu snapshot, HUD snapshot, tactical panel snapshot, build console snapshot, settings modal snapshot, training lab snapshot, game-over snapshot, wave-result snapshot, activity-log snapshot, tooltip snapshot, tutorial snapshot, mobile-action snapshot, core, power, wave 이벤트를 Preact signals로 단방향 연결합니다. Preact MainMenu/BuildConsole의 클릭은 request 이벤트만 발행하고, 실제 시작/난이도/선택/카테고리 변경은 기존 Scene/`UIManager` 경로가 처리합니다. Preact SettingsModal은 display payload의 localized labels로 game/audio/graphics/system 탭 UI를 제공하고 설정/저장 요청 이벤트만 발행하며 실제 적용은 기존 `SettingsUI`와 저장 경로가 담당합니다. Preact TrainingLabModal은 기존 Lab snapshot을 progress bar로 표시하고 DEFENSE/SYSTEM 탭 request만 발행하며, 실제 tab render/job 선택/auto 조작은 기존 `TrainingLabUI`가 담당합니다. Preact GameOverScreen은 display payload의 localized run summary labels를 표시하고 restart/main-menu request를 발행하며 `UIManager`가 legacy reload 경로로 처리합니다. Preact WaveResultCard는 display payload의 localized wave result labels를 표시하고 close request로 Preact signal만 닫고, ActivityLog는 display payload의 localized labels와 snapshot history toggle을 제공합니다. Preact Tooltip은 display payload의 position/close label을 표시하고 데스크톱 위치 mirror와 모바일 bottom-sheet mirror를 제공하되 close는 기존 `UIManager.hideTooltip()` 경로로 처리합니다. TutorialPanel은 `src/ui/tutorialPanelDisplay.ts`의 localized labels 포함 common display payload snapshot을 표시하고 skip request만 기존 `completeAll()` 경로로 보냅니다. Legacy tutorial panel DOM render/typewriter/skip binding은 같은 payload의 legacy 입력을 받아 `src/ui/legacyTutorialPanel.ts`가 담당하고 Phaser guide highlights는 계속 `TutorialManager`/Phaser Graphics가 렌더합니다. MobileActionBar/MobileBuildSummary는 일반 모바일에서 실제 조작 표면이며 기존 mobile DOM은 hidden shadow fallback으로 동기화되고, short landscape에서는 기존 legacy fallback이 남습니다.
3. 시작 시 `MainScene`으로 전환하면 `UIManager.ensureLegacyHudShell()`이 `src/ui/legacyHudDom.ts`를 호출해 빈 `#game-hud-shell` 안에 호환용 legacy top/right/build DOM IDs를 생성하고, settings/game-over/tooltip/activity/notification fallback roots도 런타임 생성합니다. 데스크톱에서는 `#top-hud`, `#hud-right-rail`, `#bottom-ui-container`, `#build-console`이 hidden shadow fallback이고 Preact TopBar/RightRail/BuildConsole이 실제 표면입니다.
4. `UIManager`가 기존 DOM 상태 sync와 snapshot 발행, request 중개를 담당합니다. Legacy shell shadow sync, ESC fallback modal hide, speed button active sync는 `src/ui/legacyHudDom.ts`, Top HUD localized labels/resource/power/wave/timer 표시 payload 조립은 `src/ui/topHudDisplay.ts`, Top HUD legacy stat mirror는 `src/ui/legacyTopHud.ts`, tactical labels/objective/wave/power/defense 표시 payload와 RightRail common display payload 조립은 `src/ui/tacticalPanelDisplay.ts`, tactical panel legacy DOM refs/update는 `src/ui/legacyTacticalPanels.ts`, legacy build console DOM render/selection sync는 `src/ui/legacyBuildConsole.ts`, BuildConsole localized labels/snapshot/selected-tool legacy panel display payload 조립은 `src/ui/buildConsoleSnapshot.ts`, wave result localized labels/snapshot/card/log display payload 조립은 `src/ui/waveResultDisplay.ts`, game-over localized labels/snapshot/stat line display payload 조립은 `src/ui/gameOverDisplay.ts`, tooltip open/close label/activity snapshot, desktop/mobile tooltip legacy mirror input, activity log localized labels/legacy entry, 로그 표시 문구 조립은 `src/ui/notificationDisplay.ts`, game-over legacy open/restart/stats DOM 반영은 `src/ui/legacyGameOver.ts`, wave result/log/tooltip의 legacy append/position/remove/hide는 `src/ui/legacyNotifications.ts`, mobile layout/canvas focus DOM environment는 `src/ui/domEnvironment.ts`가 처리하고, `index.html`은 Phaser mount, Preact root, 빈 legacy shell만 직접 제공합니다. Top HUD labels, resource, power, wave-start/timer 표시는 `src/ui/topHudDisplay.ts`의 공통 display payload/snapshot helper를 통해 legacy mirror와 Preact snapshot이 같은 포맷을 쓴다.
5. 설정과 Neural Operations Lab UI는 각각 `SettingsUI`, `TrainingLabUI`에 위임됩니다. `SettingsUI`는 실제 설정 적용과 request 처리를 유지하고, `src/ui/settingsDisplay.ts`가 current settings state와 localized labels를 legacy input/open state와 Preact snapshot 공통 display payload로 변환합니다. Legacy settings DOM refs/guard와 DOM sync 적용은 `src/ui/legacySettings.ts`가 담당합니다. `TrainingLabUI`는 Lab job/auto/reward request 처리를 유지하고, `src/ui/trainingLabDisplay.ts`가 Lab overview/planner/duration, Defense/System row view-model과 legacy shell/Preact snapshot 공통 display payload 변환을 담당합니다. Legacy Lab modal 생성/open shadow/shell/row render는 `src/ui/legacyTrainingLab.ts`가 담당합니다. 별도 레거시 research modal은 제거되었고, Preact research shortcut은 기존 `RESEARCH_OPENED` request를 통해 Training Lab SYSTEM 탭으로 연결됩니다.
6. 모바일은 `MainScene.updateMobileLayoutState()`가 `src/ui/domEnvironment.ts`의 media query/body class helper로 layout 상태를 동기화하고, `MobileUIManager`가 액션바/케이블 메뉴/빌드 요약 snapshot과 action request 처리를 갱신합니다. `src/ui/mobileActionDisplay.ts`가 selected tool/action/cable-menu state와 localized aria/menu labels를 legacy active/summary와 Preact snapshot 공통 display payload로 변환하고, legacy mobile action/cable/build-summary DOM 생성, active class, cable menu class mirror, shadow sync는 `src/ui/legacyMobileControls.ts`가 담당합니다. mobile/short-landscape 판정도 `src/ui/domEnvironment.ts`를 사용합니다. 일반 모바일 portrait에서는 Preact MobileActionBar/MobileBuildSummary와 Preact BuildConsole이 실제 표면이고 legacy mobile action/build DOM은 shadow 상태로 남습니다. Short landscape에서는 Preact mobile/build layer가 숨겨져 기존 DOM fallback이 계속 조작 표면입니다.
