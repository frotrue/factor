...
| `src/managers/MapManager.ts` | 자원 패치와 BLOCKER 지형 생성, map/build/camera bounds 조회, 작은 튜토리얼 arena 맵, 캠페인 seed 맵 | 초기 맵, 튜토리얼 맵, 저장 복원, 적 경로 차단, 배치/카메라 제한 | standard preset은 `-64..64` world/build bounds와 `-60..60` resource bounds를 가진 큰 유한 맵. `RESOURCE_RINGS`가 있으면 중반 거리대에 랜덤 자원을 집중하고, 없으면 기존 사각 range 랜덤 자원 fallback을 사용. bounds 필드는 `MainScene.isBlocked()`와 `CameraController`가 사용 | `src/managers/MapManager.test.ts` |
