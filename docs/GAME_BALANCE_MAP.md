...
| `MATERIAL_SAMPLE` | Material Insight 생산 입력, 채굴 루프에서 주기적으로 획득 | `Miner`, `ResearchLab` |
| `TACTICAL_DATA` | Tactical Insight 생산 입력, 데이터 다운로드 루프에서 주기적으로 획득 | `DataDownloader`, `ModelTrainingLab` |
...
| 3종 Insight | material/tactical/system 연구 진행 재료 | `ResearchManager`, `ResearchLab`, `ModelTrainingLab`, `DataCenter` |
...
Core: 데이터 수신량 집계
ModelTrainingLab: RAW_DATA/LABELED_DATA/WEIGHT_UPDATE 소비 -> 방어 모델 훈련
ResearchLab: MATERIAL_SAMPLE -> Material Insight
ModelTrainingLab: TACTICAL_DATA -> Tactical Insight
DataCenter: 시스템 작동 로그 -> System Insight
ResearchManager: Insight buffer + throughput 소비 -> 전역 연구 완료/효과 적용
```

물리 자원 루프는 `Miner -> Conveyor/FastLink -> Storage` 중심이며, 현재 컨베이어는 `SILICON`만 운반합니다. 케이블도 `SILICON`을 Storage로 보낼 수 있고, 케이블/컨베이어는 이미 버퍼에 들어온 아이템 이동 자체는 endpoint 전력과 분리합니다. 전력은 생산/가공 속도를 제어하고, Repeater/AP 같은 능동 중계는 전력이 필요합니다. 데이터 아이템은 케이블 또는 AP 릴레이를 통해 이동합니다.
...
| 방어 모델 | 시작 정확도 40, 플레이어가 Lab에서 선택한 보상 모드에 따라 학습 완료 시 정확도 +10 또는 공격력 +5. 학습 시간은 이번 학습에 소모한 데이터량 / `INITIAL_DATA_REQUIREMENT` 비율만큼 증가 | `MainScene`, `ModelTrainingLab`, `DefenseTower`, `modelTrainingProgress` |
| GPU 가속 | 모델 정확도 100 달성 후 해금, 인접/전력 필요, 최대 4개 x 학습시간 -20% | `GPU_CLUSTER`, `ModelTrainingLab`, `modelTrainingProgress` |
| 독립 연구 | 6축 노드, 3종 Insight 비용, 슬롯, throughput, 선행조건, 효과 | `CONFIG.RESEARCH_AXES`, `CONFIG.RESEARCH_SETTINGS`, `CONFIG.RESEARCH`, `ResearchManager`, `ResearchPanel` |
...
|---|---|---|---|
| `CLASSIFIER` | 단일 대상, lock-on 계열 공격 | `CONFIG.BUILDINGS.CLASSIFIER.DEFENSE`, `DefenseTower` | 초반 핵심 방어, 명중률이 모델 정확도에 민감 |
...
| 모델 정확도/공격력 | 정확도는 명중률/기본 모델 계수, 공격력 보너스는 후반 피해 배율 | `MainScene.defenseModelStates`, `DefenseTower.tryFire()` | 훈련 랩 밸런스가 방어 전체 DPS에 영향 |

방어 피해는 대략 `기본 DAMAGE * 연구 피해 배율 * (0.6 + effectiveModelAccuracy / 125) * (1 + damageBonus / 100)`로 계산됩니다. `effectiveModelAccuracy`는 저장된 `modelAccuracy`에서 게임 시간 `CONFIG.MODEL_TRAINING.ACCURACY_DECAY_INTERVAL_MS`마다 `ACCURACY_DECAY_PER_INTERVAL`만큼 감소한 값이며, `MIN_EFFECTIVE_ACCURACY` 아래로는 내려가지 않습니다. 명중률도 이 실효 정확도에 따라 오릅니다.
...
| 연구 pacing | Insight 생산량, buffer, throughput, 연구 비용 | `ResearchManager`, `ResearchLab`, `ModelTrainingLab`, `DataCenter`, `ResearchPanel` |
...
- 전력 소비를 높이면 네트워크 satisfaction이 낮아지고 생산, 처리, AP, 방어, Insight 생산, 연구 throughput이 비례 감속됩니다. 이미 출력 버퍼에 들어온 아이템의 유선 케이블/컨베이어 이동은 계속됩니다.
